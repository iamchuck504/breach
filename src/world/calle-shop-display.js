...
export function calleShopDisplay(style) {
  ...
  for(const y of (style==='laundry'?[]:[130,235])){
    ...
    for(let i=0;i<8;i++){
      ...
      } else if(style==='stationery') {
        g.fillStyle=['#d0c5a6','#849696','#b28d6b'][i%3];
        g.fillRect(x-17,y-18-(i%3)*8,34,16+(i%3)*8);
        g.strokeStyle='#e1dac5';g.lineWidth=2;
        for(let j=0;j<3;j++){g.beginPath();g.moveTo(x-16,y-5-j*4);g.lineTo(x+16,y-5-j*4);g.stroke();}
      } else {
  ...
  if(style==='laundry')for(const x of [114,256,398]){
    g.fillStyle='#132126';g.fillRect(x-59,156,124,147);
    g.fillStyle='#899b97';g.fillRect(x-61,150,122,144);
    g.fillStyle='#394e56';g.fillRect(x-51,160,102,20);
    g.fillStyle='#c8c9b2';g.fillRect(x-40,165,25,7);
    g.fillStyle='#0d202a';g.beginPath();g.arc(x,232,38,0,Math.PI*2);g.fill();
    g.strokeStyle='#647e87';g.lineWidth=8;g.stroke();
    g.fillStyle='#4c6975';g.beginPath();g.ellipse(x+5,241,24,13,-.3,0,Math.PI*2);g.fill();
  }
