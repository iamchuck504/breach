...
for i,b in enumerate(data['boxes']):
    ...
    if b.get('walkSurface') and not b.get('deck'):
        ...
        for z in range(-13,14):box('Gallery floor paving',b['x'],3.003,z,b['w'],.006,.014,joint)
        for dx in (-.95,.95):box('Gallery floor paving',b['x']+dx,3.003,0,.014,.006,28,joint)
    elif b.get('roof'):
        # Tile seams lie on the actual roof band, not a coplanar overlay.
        for z in range(-24,25):box('Tile course',b['x'],b['h']+.004,z,b['w']-.005,.008,.018,joint)
    elif b['d']>6:
...
for side in (-1,1):
    # Nine framed open windows per side. No glass or false black planes.
    for i in range(-4,5):
        z=i*3
    ...
    for end in (-1,1):
        for z in (21.81,23.98):box('Stair entrance jamb',side*21.40,1.49,end*z,.85,2.98,.075,trim)
        box('Stair entrance lintel',side*21.40,3.04,end*22.9,.85,.08,2.2,trim)
